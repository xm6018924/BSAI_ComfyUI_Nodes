...
class LingBotVideoPipeline(DiffusionPipeline):
    ...
    def encode_video_latent(
        ...
        # Ensure VAE is fully on CUDA before encode (model_cpu_offload may have moved it)
        target_device = video.device if video.device.type == "cuda" else torch.device("cuda")
    ...
    def _decode_latents(
        ...
        # When model_cpu_offload is enabled, the VAE may have been moved to CPU
        # after denoising. Force it back to CUDA so that all sub-modules are on
        # the same device — otherwise Conv3d kernels can receive a mix of
        # CPU weights and CUDA tensors, triggering:
        #   "Could not run 'aten::slow_conv3d_forward' with arguments from the 'CUDA' backend"
        target_device = latents.device if latents.device.type == "cuda" else torch.device("cuda")
        self.vae.to(target_device)

        vae_device = _module_device(self.vae)
        vae_dtype = _module_dtype(self.vae)
        # VAE decode on GPU with bf16/fp16 weights:
        # Disable autocast to keep native bf16/fp16 execution on CUDA,
        # preventing slow_conv3d CPU fallback caused by dtype mismatch
        # between autocast-fp32 input and bf16 weights.
        if vae_device.type == "cuda" and vae_dtype in {torch.bfloat16, torch.float16}:
            vae_latents = self._dit_latent_to_vae(latents).to(device=vae_device, dtype=vae_dtype)
        else:
